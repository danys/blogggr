...
public class FriendsControllerTest {
  ...
  public void createFriendship_Normal() throws Exception {
    ...
    mvc.perform(post(BASE_URL + "/friends")
        ...
        .andExpect(header().string(AppConfig.LOCATION_HEADER_KEY,
            AppConfig.FULL_BASE_URL + "/friends/" + friend.getId().getUserOneId() + '/' + friend
                .getId().getUserTwoId()));
  }
  ...
  public void updateFriendship_Normal() throws Exception {
    ...
    doNothing().when(friendService).updateFriend(any(Long.class), any(Long.class), any(Long.class),
        any(FriendDataUpdate.class));
    mvc.perform(put(BASE_URL + "/friends/1/2")
        .with(user(createUserPrincipal("dan@dan.com", 1L)))
        .contentType(MediaType.APPLICATION_JSON)
  ...
  public void getFriends_Normal() throws Exception {
    ...
    mvc.perform(get(BASE_URL + "/friends")
        ...
        .andExpect(status().isOk())
        .andExpect(content().json(
            "{'apiVersion': '1.0', 'data': [{'userId': 10, 'email': 'dany@dan.com', 'firstName': 'daniel', 'lastName': 'Müller'}]}"));
  }
