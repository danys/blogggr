...
public class GetPostByLabelValidator extends GenericValidator{
    ...
    protected boolean validate(Map<String,String> input, String body){
        if (!input.containsKey(userIDKey) || !input.containsKey(postShortNameKey)) return false;
        if (!stringIsNumber(input.get(userIDKey))) return false;
        if (input.get(postShortNameKey).length()<minShortNameLength) return false;
        return true;
    }
}
